...
1. As the rate is dependent on both $P_{\ce{C2H4}}$ and $P_{\ce{O2}}$, we need to
   ...
   (Identifies need for decoupling (1 marks)).
...
6. Steps to find $\beta$: Similar to step 4. \hfill (1 marks)
...
df['Rate ($mol/dm^3\\ min$)'] = rate
df['Rate ($mol/dm^3\\ s$)'] = rate/60.0
...
Rate calculate either as $mol/dm^3\ min$ or $mol/dm^3\ s$ is okay.
...
$\alpha$ = `{python} f'{alpha:.2f}'`  \hfill(3 marks - 0 marks for wrong value)

$\beta$ = `{python} f'{beta:.2f}'`  \hfill(3 marks - 0 marks for wrong value)
